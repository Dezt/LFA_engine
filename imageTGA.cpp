...
namespace LFA
{
	...
	int imageTGA::Load(const char* szFilename)
	{
		using namespace std;
		ifstream fIn;
		...
		// Check flip bit
		// PPP: Not sure why this is being done. Commented for now
		//if((pData[17] & 0x20)==0) 
		//	FlipImg();
	...
	int imageTGA::ReadHeader() // Examine the header and populate our class attributes
	{
	...
	int imageTGA::LoadRawData() // Load uncompressed image data
	{
		short iOffset;
	...
	int imageTGA::LoadTgaRLEData() // Load RLE compressed image data
	{
	...
	int imageTGA::LoadTgaPalette() // Load a 256 color palette
	{
		unsigned char bTemp;
	...
	void imageTGA::BGRtoRGB() // Convert BGR to RGB (or back again)
	{
	...
	void imageTGA::FlipImg() // Flips the image vertically (Why store images upside down?)
	{
		unsigned char bTemp;
	...
	int imageTGA::GetBitsPerPixel()  const
	{
		return iBPP;
	}
	...
	int imageTGA::GetHeight() const
	{
		return iHeight;
	}


	unsigned char* imageTGA::GetImg() const
	{
		return pImage;
	}


	unsigned char* imageTGA::GetPalette()
	{
		return pPalette;
	}

	unsigned long imageTGA::getImgSize()
	{
		return lImageSize;
	}
}
